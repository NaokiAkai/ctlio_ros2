  ...
  const lico::State& state)
{
  ...
  const Eigen::Quaternionf q(state.R.matrix());
  ...
  const Eigen::Vector3f v = state.R.matrix().transpose() * state.v;

  msg.twist.twist.linear.x = v.x();
  msg.twist.twist.linear.y = v.y();
  msg.twist.twist.linear.z = v.z();
  msg.twist.twist.angular.x = state.w.x();
  msg.twist.twist.angular.y = state.w.y();
  msg.twist.twist.angular.z = state.w.z();
